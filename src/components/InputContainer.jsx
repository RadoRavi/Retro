...
console.log("section",section,"comments",myComments)
    ...
    function sendData(event) {
        ...
        var randomKey=null
        if(onEdit){            
            const id2 =event.currentTarget.parentElement.parentElement.parentElement.id
    ...
    return(
        ...
        <div className="inputContainer">            <div style={{width:"90%"}}>
        {onEdit? 
   <TextField
 ...
 />:<input className="textarea" role="textbox" type='text' id="standard-basic" label="Standard" variant="standard" onChange={updateTypo}  placeholder={!haveDefault&&"add your reflections here"} value={typo} contenteditable/>}
        ...
        {typo&&<button id={section} className="resSend" onClick={sendData} variant="outlined">{onEdit?"Update":<FontAwesomeIcon icon={faPaperPlane} style={{color: "#8cb0ee",}} />}</button>}
        </div>
